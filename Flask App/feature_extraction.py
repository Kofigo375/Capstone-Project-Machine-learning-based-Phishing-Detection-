...
def generate_dataset(url):
   ...
   # global_rank=find_global_rank(domain)         # come back to find an alternative for checkpagerank.net  # noqa: E501
    data_set=[]
    ...
    data_set.append(favicon(soup,url,domain))  # dropped when training
    ...
    data_set.append(submitting_to_email(soup))    # droped
    data_set.append(abnormal_url(domain,url))
    data_set.append(redirect(url))                  # droped when training
    data_set.append(on_mouse_over(url))
    data_set.append(right_click(url))                 # droped when trianing
    data_set.append(popup_window(url))               # droped
    data_set.append(iframe(url))                    # droped when training 
    data_set.append(age_of_domain(response))
    data_set.append(dns_record(domain))
    #data_set.append(web_traffic(url))      # alexa wans't working 
...
# checking if function can extract data from   url and return dataset



"""
try:
    features = generate_dataset("facebook.com")
